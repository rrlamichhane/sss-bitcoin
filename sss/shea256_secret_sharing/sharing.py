...
def secret_int_to_points(
    secret_int: int, point_threshold: int, num_points: int, prime: Optional[int] = None
) -> List[Tuple[int, int]]:
...
def points_to_secret_int(
    points: List[Tuple[int, int]], prime: Optional[int] = None
) -> int:
...
def point_to_share_string(point: Tuple[int, int], charset: str) -> str:
    ...
    if not (
        ...
        and len(point) == 2
        and isinstance(point[0], integer_types)
        and isinstance(point[1], integer_types)
    ):
...
def share_string_to_point(share_string: str, charset: str) -> Tuple[int, int]:
...
class SecretSharer:
    ...
    secret_charset: str = string.hexdigits[0:16]
    share_charset: str = string.hexdigits[0:16]
    ...
    @classmethod
    def split_secret(
        cls, secret_string: str, share_threshold: int, num_shares: int
    ) -> List[str]:
...
class PlaintextToHexSecretSharer(SecretSharer):
    ...
    secret_charset: str = string.printable
    share_charset: str = string.hexdigits[0:16]
...
class BitcoinToB58SecretSharer(SecretSharer):
    ...
    secret_charset: str = base58_chars
    share_charset: str = base58_chars
...
class BitcoinToB32SecretSharer(SecretSharer):
    ...
    secret_charset: str = base58_chars
    share_charset: str = base32_chars
...
class BitcoinToZB32SecretSharer(SecretSharer):
    ...
    secret_charset: str = base58_chars
    share_charset: str = zbase32_chars
