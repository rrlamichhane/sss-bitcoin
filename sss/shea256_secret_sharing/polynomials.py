...
def random_polynomial(degree: int, intercept: int, upper_bound: int) -> List[int]:
...
def get_polynomial_points(
    coefficients: List[int], num_points: int, prime: int
) -> List[Tuple[int, int]]:
...
def modular_lagrange_interpolation(
    x: int, points: List[Tuple[int, int]], prime: int
) -> int:
